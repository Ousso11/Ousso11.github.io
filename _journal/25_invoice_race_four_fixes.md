---
title: "Four Fixes to One Invoice Race, Each One Revealing the Next"
collection: journal
...
A small API consequence worth noting: the read had to be widened to return `updated_at`. **The token you lock on has to come back from the read** — optimistic locking is a property of the pair, not of the write.
...
- Removing an exemption **deletes a row in a different table**, which never touches the invoice — so `updated_at` on the invoice cannot possibly see it. No amount of row versioning helps. That one needed post-write verification and self-healing: after waiving, re-check the exemption and reinstate if it has gone. And the reinstated amount had to be derived from the freshly re-fetched row, because a concurrent fold had added usage the earlier snapshot did not contain.

## 💡 Takeaway

- **Guarding on status and guarding on version solve different problems.** Status stops illegal transitions; a version token stops lost increments. Money code usually needs both.
